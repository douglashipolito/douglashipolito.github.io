...
module Jekyll
  class CodePen < Liquid::Tag
    def initialize(tag_name, markup, tokens)
      if /(?<pen>\w+)(?:\s(?<user>\w+))(?:\s(?<themeid>\w+))?(?:\s(?<type>\w+))?(?:\s(?<height>\d+))?/ =~ markup
        @pen      = pen
        @user     = user
        @themeid  = themeid || '5971'
        @type     = type || 'result'
        @height   = height || '300'
      end
    end
    ...
    def render(context)
      if @pen && @user
        "<p data-height=\"#{@height}\" data-theme-id=\"#{@themeid}\" data-href=\"#{@pen}\" data-user=\"#{@user}\" data-default-tab=\"#{@type}\" class='codepen'>See more like <a href='http://codepen.io/#{@user}/pen/#{@pen}/'>this Pen</a> on <a href='http://codepen.io'>CodePen</a>.</p><script async src=\"//codepen.io/assets/embed/ei.js\"></script>"
      else
